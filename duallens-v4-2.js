...
function image(img,x,y,w,h){
  ...
  // Match width: always fill the full lens-card width.
  // Taller images crop vertically, but never shrink into a narrow strip.
  ...
  ctx.restore();
}const s=Math.min(w/img.naturalWidth,h/img.naturalHeight),dw=img.naturalWidth*s,dh=img.naturalHeight*s,dx=x+(w-dw)/2,dy=y+(h-dh)/2;ctx.save();ctx.beginPath();ctx.roundRect(x,y,w,h,16);ctx.clip();ctx.fillStyle="#fff";ctx.fillRect(x,y,w,h);ctx.drawImage(img,dx,dy,dw,dh);ctx.restore()}
