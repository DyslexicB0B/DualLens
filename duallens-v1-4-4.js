...
function updatePreview() {
  ...
  $("leftSourceChip").href =
    $("leftSourceUrl").value.trim() || "#";

  $("rightSourceChip").textContent =
    $("rightSourceLabel").value.trim() || "Right source";
  $("rightSourceChip").href =
    $("rightSourceUrl").value.trim() || "#";
}

function openSource(side) {
  const url = $(side + "SourceUrl").value.trim();

  if (!url) {
    $("message").textContent = "Add the original source URL first.";
    return;
  }
...
function renderCanvas() {
  ...
  ctx.beginPath();
  ctx.arc(W / 2 - 16, 48, 21, 0, Math.PI * 2);
  ctx.fill();
  ...
  ctx.fillText("DualLens", W / 2, 64);

  ctx.font = "900 14px Arial";
  ctx.fillStyle = "#667085";
  ctx.fillText("SAME STORY. DIFFERENT LENS.", W / 2, 90);
  ...
  roundedRect(
    leftX, top, col, frameHeight, 22,
    "#fff", "rgba(24,95,230,.4)", 3
  );

  roundedRect(
    rightX, top, col, frameHeight, 22,
    "#fff", "rgba(239,48,56,.4)", 3
  );
  ...
  ctx.fillText("DualLens", W / 2, 1265);
  ctx.textAlign = "left";
}
...
async function sharePost() {
  ...
  if (!blob) {
    $("message").textContent = "Share failed.";
    return;
  }

  const file = new File([blob], "duallens-comparison.png", {
    type: "image/png",
  });
  ...
  downloadBlob(blob);
  $("message").textContent =
    "Native sharing is unavailable, so the PNG was downloaded.";
}
...
function openXComposer() {
  ...
  $("message").textContent =
    "X opened with your text. Attach the copied or downloaded image.";
}
...
function draftData() {
  return {
    name: $("draftName").value.trim() || "Untitled comparison",
    headline: $("headline").value,
    leftLabel: $("leftLabel").value,
    rightLabel: $("rightLabel").value,
    leftSourceLabel: $("leftSourceLabel").value,
    rightSourceLabel: $("rightSourceLabel").value,
    leftSourceUrl: $("leftSourceUrl").value,
    rightSourceUrl: $("rightSourceUrl").value,
    leftFit: $("leftFit").value,
    rightFit: $("rightFit").value,
    postCopy: $("postCopy").value,
    leftImage: $("leftPreviewImage").src || "",
    rightImage: $("rightPreviewImage").src || "",
...
function saveDraft() {
  const drafts = JSON.parse(
    localStorage.getItem("duallensV141Drafts") || "[]"
  );
...
function loadDraft(draft) {
  $("draftName").value = draft.name || "Untitled comparison";
  $("headline").value = draft.headline || "";
  $("leftLabel").value = draft.leftLabel || "";
  $("rightLabel").value = draft.rightLabel || "";
  $("leftSourceLabel").value = draft.leftSourceLabel || "";
  $("rightSourceLabel").value = draft.rightSourceLabel || "";
  $("leftSourceUrl").value = draft.leftSourceUrl || "";
  $("rightSourceUrl").value = draft.rightSourceUrl || "";
  $("leftFit").value = draft.leftFit || "contain";
  $("rightFit").value = draft.rightFit || "contain";
  $("postCopy").value = draft.postCopy || "";

  const leftImage = $("leftPreviewImage");
  const rightImage = $("rightPreviewImage");

  if (draft.leftImage) {
    leftImage.src = draft.leftImage;
  ...
  if (draft.rightImage) {
    rightImage.src = draft.rightImage;
...
function renderDrafts() {
  ...
  if (!drafts.length) {
    container.innerHTML =
      '<span class="message">No saved drafts yet.</span>';
    return;
  }
  ...
  drafts.forEach((draft, index) => {
    ...
    loadButton.className = "secondary draft-load";
    loadButton.textContent = draft.name || "Untitled comparison";
    loadButton.addEventListener("click", () => loadDraft(draft));

    const deleteButton = document.createElement("button");
    deleteButton.className = "secondary draft-delete";
...
$("leftOpenSourceBtn").addEventListener("click", () => openSource("left"));
$("rightOpenSourceBtn").addEventListener("click", () => openSource("right"));
