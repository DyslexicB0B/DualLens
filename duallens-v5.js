...
function image(img,x,y,w,h,mode="full",zoom=1){
  ...
  if(mode==="full"){
    ...
    const dx=x+pad+(sw-dw)/2, dy=y+pad+(sh-dh)/2;
...
function drawStackScreenshot(img,x,y,w,h,mode="readable",zoom=1){
  ...
  ctx.fillRect(x,y,w,h);
  if(mode==="full"){
    const pad=8, sw=w-pad*2, sh=h-pad*2;
    const scale=Math.min(sw/img.naturalWidth,sh/img.naturalHeight);
    const dw=img.naturalWidth*scale, dh=img.naturalHeight*scale;
    ctx.drawImage(img,0,0,img.naturalWidth,img.naturalHeight,x+pad+(sw-dw)/2,y+pad+(sh-dh)/2,dw,dh);
  }else{
    const scale=(w/img.naturalWidth)*zoom;
    const dw=img.naturalWidth*scale, dh=img.naturalHeight*scale;
    ctx.drawImage(img,0,0,img.naturalWidth,img.naturalHeight,x+(w-dw)/2,y+(h-dh)/2,dw,dh);
  }
  ctx.restore();
}
...
function renderStacked(){
  ...
  const lg=ctx.createRadialGradient(0,650,20,0,650,800);
  lg.addColorStop(0,"rgba(24,95,230,.13)");lg.addColorStop(.58,"rgba(24,95,230,.05)");lg.addColorStop(1,"rgba(24,95,230,0)");
  ctx.fillStyle=lg;ctx.fillRect(0,0,W,H);
  const rg=ctx.createRadialGradient(W,650,20,W,650,800);
  rg.addColorStop(0,"rgba(239,48,56,.13)");rg.addColorStop(.58,"rgba(239,48,56,.05)");rg.addColorStop(1,"rgba(239,48,56,0)");
  ctx.fillStyle=rg;ctx.fillRect(0,0,W,H);

  logo(315,70,1.12);
  ctx.fillStyle="#071229";ctx.font='900 72px "Roboto Condensed",Arial';ctx.fillText("DualLens",385,96);
  ctx.font='800 16px Arial';ctx.fillStyle="#344054";ctx.fillText("SAME STORY. DIFFERENT LENS.",385,130);

  text($("headline").value.trim().toUpperCase(),540,220,970,76,'900 italic 78px "Roboto Condensed",Arial',"#071229",2,"center");

  ctx.textAlign="center";ctx.font='900 18px "Roboto Condensed",Arial';ctx.fillStyle="#071229";ctx.fillText("PERSPECTIVE DISTANCE",540,350);
  ctx.strokeStyle="#aeb8c8";ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(175,340);ctx.lineTo(390,340);ctx.moveTo(690,340);ctx.lineTo(905,340);ctx.stroke();
  meter($("distanceScore").value,285,370,410);
  ctx.font='900 19px "Roboto Condensed",Arial';ctx.fillStyle="#344054";ctx.fillText($("distanceCaption").value.trim().toUpperCase(),540,430);

  const x=28,w=1024,shotX=44,shotW=992,shotH=545;
  let y=475;

  rr(x,y,w,72,24,"#185fe6");ctx.fillStyle="#185fe6";ctx.fillRect(x,y+46,w,26);
  ctx.font='900 italic 38px "Roboto Condensed",Arial';ctx.fillStyle="#fff";ctx.fillText("LEFT LENS",540,y+48);
  y+=84;
  drawStackScreenshot(state.leftImage,shotX,y,shotW,shotH,state.leftMode,state.leftZoom);
  y+=shotH+24;
  ctx.font='900 24px Arial';ctx.fillStyle="#185fe6";ctx.fillText(($("leftName").value||"Left source")+"  ↗",540,y);

  y+=58;
  ctx.strokeStyle="#aeb8c8";ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(120,y);ctx.lineTo(420,y);ctx.moveTo(660,y);ctx.lineTo(960,y);ctx.stroke();
  rr(468,y-30,144,60,30,"#167a9d","#071229",2);ctx.font='900 26px "Roboto Condensed",Arial';ctx.fillStyle="#fff";ctx.fillText("VS",540,y+9);

  y+=64;
  rr(x,y,w,72,24,"#ef3038");ctx.fillStyle="#ef3038";ctx.fillRect(x,y+46,w,26);
  ctx.font='900 italic 38px "Roboto Condensed",Arial';ctx.fillStyle="#fff";ctx.fillText("RIGHT LENS",540,y+48);
  y+=84;
  drawStackScreenshot(state.rightImage,shotX,y,shotW,shotH,state.rightMode,state.rightZoom);
  y+=shotH+24;
  ctx.font='900 24px Arial';ctx.fillStyle="#ef3038";ctx.fillText(($("rightName").value||"Right source")+"  ↗",540,y);

  ctx.fillStyle="#071229";ctx.fillRect(0,H-74,W,74);
  ctx.strokeStyle="#185fe6";ctx.beginPath();ctx.moveTo(35,H-36);ctx.lineTo(390,H-36);ctx.stroke();
  ctx.strokeStyle="#ef3038";ctx.beginPath();ctx.moveTo(690,H-36);ctx.lineTo(1045,H-36);ctx.stroke();
  ctx.font='900 28px "Roboto Condensed",Arial';ctx.fillStyle="#fff";ctx.fillText("duallens.ai",540,H-26);
  ctx.textAlign="left";
}
