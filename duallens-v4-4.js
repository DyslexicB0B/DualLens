...
function render(){
  ...
  // Subtle contour lines / editorial texture.
  ctx.save();
  ctx.globalAlpha=.075;
  ctx.lineWidth=1.5;
  ...
  // Very light halftone dots near upper corners.
  ctx.globalAlpha=.045;
  ...
  // Brand shifted right for optical centering and slightly lower.
  logo(318,76,1.15);
  ...
  // Headline moved down while preserving current type treatment.
  text(
  ...
  // Perspective section slightly lower to tighten the large white gap.
  ctx.textAlign="center";
  ...
  // Existing lens-card layout preserved.
  const lx=28,rx=552,cy=500,cw=500,ch=728;
  ...
  // Same screenshot behavior; only vertical room adjusted to retain footer.
  const imageY=cy+100;
  const imageH=528;
  ...
  ctx.fillStyle="#185fe6";
  ctx.fillText(($("leftName").value||"Left source")+"  ↗",lx+cw/2,cy+684);

  ctx.fillStyle="#ef3038";
  ctx.fillText(($("rightName").value||"Right source")+"  ↗",rx+cw/2,cy+684);

  rr(489,800,102,102,51,"#fff","#aeb8c8",2);
  ctx.fillStyle="#071229";
  ctx.font='900 22px "Roboto Condensed",Arial';
  ctx.fillText("COMPARE",540,840);
  ctx.font='800 15px Arial';
  ctx.fillText("TWO VIEWS",540,867);
