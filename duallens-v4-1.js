...
const state={leftImage:null,rightImage:null,matchWeight:"Different"};
...
function image(img,x,y,w,h){if(!img){rr(x,y,w,h,16,"#f8fafc","#cbd5e1",2);ctx.textAlign="center";ctx.font="800 20px Arial";ctx.fillStyle="#667085";ctx.fillText("Upload screenshot",x+w/2,y+h/2);ctx.textAlign="left";return}const s=Math.min(w/img.naturalWidth,h/img.naturalHeight),dw=img.naturalWidth*s,dh=img.naturalHeight*s,dx=x+(w-dw)/2,dy=y+(h-dh)/2;ctx.save();ctx.beginPath();ctx.roundRect(x,y,w,h,16);ctx.clip();ctx.fillStyle="#fff";ctx.fillRect(x,y,w,h);ctx.drawImage(img,dx,dy,dw,dh);ctx.restore()}
...
ctx.fillText("MATCH: "+state.matchWeight.toUpperCase(),540,420);
const lx=24,rx=556,cy=442,cw=500,ch=568;rr(lx,cy,cw,ch,24,"#eaf1ff","#185fe6",2);rr(rx,cy,cw,ch,24,"#fff0f1","#ef3038",2);rr(lx,cy,cw,68,24,"#185fe6");ctx.fillStyle="#185fe6";ctx.fillRect(lx,cy+44,cw,24);rr(rx,cy,cw,68,24,"#ef3038");ctx.fillStyle="#ef3038";ctx.fillRect(rx,cy+44,cw,24);ctx.font='900 30px "Roboto Condensed",Arial';ctx.fillStyle="#fff";ctx.textAlign="center";ctx.fillText("LEFT LENS",lx+cw/2,cy+44);ctx.fillText("RIGHT LENS",rx+cw/2,cy+44);image(state.leftImage,lx+18,cy+88,cw-36,420);image(state.rightImage,rx+18,cy+88,cw-36,420);ctx.font="900 18px Arial";
...
document.querySelectorAll(".match-btn").forEach(btn=>{
  btn.addEventListener("click",()=>{
    ...
    state.matchWeight=btn.dataset.weight;
    $("matchWeightValue").value=btn.dataset.weight;
    render();
  });
});
