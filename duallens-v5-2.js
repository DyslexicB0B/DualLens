...
function image(img,x,y,w,h,mode="full",zoom=1){
  ...
  if(mode==="full"){
    ...
    // Higher zoom enlarges it while keeping it anchored to the RIGHT edge.
    const pad=10;
    const sw=w-pad*2;
    const sh=h-pad*2;
    ...
    // Right-align the screenshot rather than centering it.
    const dx=x+w-pad-dw;
    const dy=y+(h-dh)/2;
...
function drawStackScreenshot(img,x,y,w,h,mode="readable",zoom=1){
  ...
  if(mode==="full"){
    ...
    // Allow zooming in while anchoring the screenshot to the RIGHT.
    const pad=8;
    const sw=w-pad*2;
    const sh=h-pad*2;
