...
function drawText(
  text, x, y, maxWidth, lineHeight, font, color,
  maxLines = 99, align = "left"
) {
  ctx.font = font;
  ctx.fillStyle = color;
  ctx.textAlign = align;
...
async function sharePost() {
  ...
  downloadBlob(blob);
  $("message").textContent =
    "Native sharing is not supported here, so the PNG was downloaded.";
}
...
function saveDraft() {
  const drafts = JSON.parse(
    localStorage.getItem("duallensV14Drafts") || "[]"
  );
